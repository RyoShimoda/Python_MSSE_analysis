...
# simple no-op writer for libraries that accept a logfile=file-like argument
class _NullWriter:
...
def read_freezing_data(files, type = "FC"):
    ...
    # devnull をループ外で一度だけ開き、各ファイル読み込み時に使い回す
    with open(os.devnull, "w") as devnull:
        for file in files:
            # 旧 .xls では pandas/xlrd が stderr に直接 WARNING を出すことがあるため抑制
            with warnings.catch_warnings():
                warnings.filterwarnings("ignore", message=".*OLE2 inconsistency.*")
                # stderr に加えて stdout も抑制する（xlrd や古いライブラリが stdout に出力する場合があるため）
                try:
                    with contextlib.redirect_stderr(devnull), contextlib.redirect_stdout(devnull):
                        df = pd.read_excel(file, engine_kwargs={"logfile": _NullWriter()})
                except ValueError:
                    # xlrd may try to write to a logfile object that was closed; if that happens,
                    # retry without redirecting stdout/stderr as a fallback.
                    df = pd.read_excel(file, engine_kwargs={"logfile": _NullWriter()})
